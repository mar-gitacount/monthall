...
def main():
    ...
    while True:
        ...
        print(type(today_date),"今日の日付タイプ")
        print(today_date)
    ...
    for item in queryitems:
        ...
        for weekly_item in weekly_items:
            ...
            ws.append([day_keys,price,f"=B{diff_input_cell+low_usenum}-B{next_cell+low_usenum}"])
            print(day_keys)
            print(price)
            print("------------")
            # 行を追加
        ...
        # 値段データ
        data = Reference(ws, min_col=2, min_row=2+low_usenum, max_col=2, max_row=max_low)
        
        # 日付データ
        categories = Reference(ws, min_col=1, min_row=3+low_usenum, max_row=max_low)
        chart.add_data(data, titles_from_data=True)
        chart.set_categories(categories)
            # 軸の反転
        chart.x_axis.reverse = True  # x軸を反転
        chart.y_axis.reverse = True  # y軸を反転


        for _ in range(low_usenum):
              ws.insert_rows(2)
        # グラフをシートに追加
